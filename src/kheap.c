...
void kheap_set_placement_address(u32 _end) { placement_address = _end; }
...
u32 kmalloc_p(u32 sz, u32 *phys) { return kmalloc_int(sz, 0, phys); }

u32 kmalloc_ap(u32 sz, u32 *phys) { return kmalloc_int(sz, 1, phys); }

u32 kmalloc(u32 sz) { return kmalloc_int(sz, 0, 0); }
...
// FIXME: convert return type to a union of NOT_FOUND and the found location
static hole_search_results find_smallest_hole(u32 size, u8 page_align,
                                              heap_t *heap) {
  hole_search_results results;
  ...
  while (i < heap->index.size) {
    ...
    if (page_align > 0) {
      ...
      if (((location + sizeof(header_t)) & 0xFFFFF000) != 0)
        offset =
            (HEAP_PAGE_SIZE - (location + sizeof(header_t))) % HEAP_PAGE_SIZE;
  ...
  // Welp we've reached the end
  if (i == heap->index.size) {
    results.notfound = -1;
    return results;
  } else {
    results.found_at_location = i;
    return results;
  }
}
...
heap_t *create_heap(u32 start, u32 end_addr, u32 max, u8 supervisor,
                    u8 readonly) {
...
void *alloc(u32 size, u8 page_align, heap_t *heap) {
  ...
  // Find the smallest hole that will fit.
  hole_search_results results = find_smallest_hole(new_size, page_align, heap);
  i32 iterator = 0;
  if (results.notfound == -1) { // If we didn't find a suitable hole
    ...
    while ((u32)iterator < heap->index.size) {
      u32 tmp = (u32)lookup_ordered_array((u32)iterator, &heap->index);
      if (tmp > value) {
        value = tmp;
        idx = iterator;
      }
    ...
    if (idx == -1) {
      ...
      header_t *header = lookup_ordered_array((u32)idx, &heap->index);
      header->size += new_length - old_length;
      // Rewrite the footer.
      footer_t *footer =
  ...
  header_t *orig_hole_header =
      (header_t *)lookup_ordered_array((u32)iterator, &heap->index);
  ...
  if (page_align && orig_hole_pos & 0xFFFFF000) {
    u32 new_location = orig_hole_pos + 0x1000 /* page size */ -
    ...
    remove_ordered_array((u32)iterator, &heap->index);
  }
  ...
  if (orig_hole_size - new_size > 0) {
    ...
    footer_t *hole_footer = (footer_t *)((u32)hole_header + orig_hole_size -
...
void free(void *p, heap_t *heap) {
  ...
  if (test_header->magic == HEAP_MAGIC && test_header->is_hole) {
    ...
    test_footer = (footer_t *)((u32)test_header + // Rewrite it's footer to
                                                  // point to our header.
