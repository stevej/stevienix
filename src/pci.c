...
u32 detect_bus() { return inl(0xCFC) != 0; }
...
u16 pci_config_read(u32 bus, u32 device, u32 func, u32 offset) {
  u32 address = ((bus << 16) | (device << 11) | (func << 8) | (offset & 0xfc) |
                 ((u32)0x80000000));
...
void print_pci_entry(PCI_VENTABLE vendor, PCI_DEVTABLE device) {
  printk("%x:%x %s %s\n", vendor.VenId, device.DevId, vendor.VenFull,
         device.ChipDesc);
}
...
PCI_DEVTABLE lookup_device(u16 vendorID, u32 deviceID) {
...
void print_pci_header_nice(struct pci_header *header) {
  ...
  print_pci_entry(lookup_vendor(header->vendor_id),
                  lookup_device(header->vendor_id, header->device_id));
}

void print_pci_header_full(struct pci_header *header) {
  printk(
      "DEVICE: vendor_id:%x device_id:%x command:%x status:%x revision_id:%x prog_if:%x subclass:%x \
...
interrupt_line:%x interrupt_pin:%x min_grant:%x max_latency:%x\n",
      header->vendor_id, header->device_id, header->command, header->status,
      header->revision_id, header->prog_if, header->subclass,
      header->class_code, header->cache_line_size, header->latency_timer,
      header->header_type, header->bist, header->bar[0], header->bar[1],
      header->bar[2], header->bar[3], header->bar[4], header->bar[5],
      header->cardbus_cis_ptr, header->subsystem_id,
      header->subsystem_vendor_id, header->rom_base_address,
      header->capabilities, header->interrupt_line, header->interrupt_pin,
      header->min_grant, header->max_latency);
}
...
/**
 * Does a brute-force enumeration of the PCI bus and prints a nice form of each
 * header found.
 *
    ...
    for (u16 device = 0; device < 32; device++) {
      for (u8 func = 0; func < 8; func++) {
        if (pci_exists(bus, device, func)) {
          struct pci_header *header = (void *)kmalloc(sizeof(pci_header));
          pci_read_header(bus, device, func, header);
          printk("[%x:%x:%x] ", bus, device, func);
          print_pci_header_nice(header);

          if (header->class_code == 0x2 && header->subclass == 0x0) {
            printk("ETH: ");
            print_pci_header_full(header);
          } else if (header->class_code == 0x3 && header->subclass == 0x0) {
            printk("VIDEO: ");
            print_pci_header_full(header);
          }
        }
