...
uint8_t ring_buffer_read(ring_buffer *buffer) {
  // TODO: synchronize this whole block around a semaphore. head must not
  // change. If head is at tail, then block until a write occurs?
  uint32_t old_head = buffer->head;
  uint8_t item = buffer->data[old_head];
...
void ring_buffer_write(ring_buffer *buffer, uint8_t item) {
  // TODO: synchronize this whole block around a semaphore. tail must not
  // change. If tail is at head then block until a read occurs?
